(function(){'use strict';var DebugUtil = {assert:function(condition, opt_message) {
  if (!condition) {
...
var GwdDoubleClickConstants = {EVENT_DETAIL_URL:"url", EVENT_DETAIL_EXIT_ID:"exit-id", EVENT_DETAIL_PRODUCT_INDEX:"product-index", EXPANDED_PAGE:"data-gwd-expanded", FULLSCREEN_CLASS_NAME:"fs", GWD_DATABINDER:"gwd-data-binder", GWD_YOUTUBE:"gwd-youtube", HANDLED:"handled", HTML_AUDIO:"audio", HTML_VIDEO:"video", START_PAGE_TRANSITION_DELAY:30, TAG_NAME:"gwd-doubleclick", GWD_SCHEMA_ID:"gwd-schema-id", Attributes:{DATA_PROVIDER:"data-provider", FULLSCREEN:"fullscreen", POLITE_LOAD:"polite-load"}, 
Events:{AD_INITIALIZED:"adinitialized", COLLAPSE_FINISH:"collapsefinish", COLLAPSE_START:"collapsestart", EXPAND_FINISH:"expandfinish", EXPAND_START:"expandstart", FULLSCREEN_SUPPORT:"fullscreensupport"}, Schemas:{DBM:"dynamic_remarketing"}};
...
GWDDynamicDataSelectorImpl.prototype.handleDomContentLoaded_ = function() {
  if (this.isUsingStudioStandardSchema_()) {
    var feedId = this.getFeedId_();
    if (hasCustomFeeds_()) {
      var dataset = gwd.dynamic.feeds[feedId];
      if (dataset) {
...
GWDDynamicDataSelectorImpl.prototype.getFeedId_ = function() {
  var feedId = this.getQueryStringParameter_(GwdDynamicDataSelectorConstants.FEED_ID_KEY), sortedFeedIds = Object.keys(gwd.dynamic.feeds).sort();
  return feedId = feedId || sortedFeedIds[0];
};
GWDDynamicDataSelectorImpl.prototype.handleWebComponentsReadyEvent_ = function() {
  var feedId = this.getFeedId_(), dataset;
  hasCustomFeeds_() && "noFeed" != feedId && (dataset = gwd.dynamic.feeds[feedId]) && this.useDataset_(dataset);
  this.populateDropdown_(feedId);
...
GWDDynamicDataSelectorImpl.prototype.populateDropdown_ = function(currentSelectionId) {
  if (hasCustomFeeds_()) {
    for (var option, sortedFeedIds = Object.keys(gwd.dynamic.feeds).sort(), i = 0; i < sortedFeedIds.length; i++) {
      var id = sortedFeedIds[i];
      option = document.createElement("option");
      option.textContent = gwd.dynamic.feeds[id].name;
      option.value = encodeURIComponent(id);
      id == currentSelectionId && option.setAttribute("selected", "");
      this.dropdown_.appendChild(option);
    }
  }
  var emptyOption = document.createElement("option");
  emptyOption.textContent = "No Feed";
  emptyOption.value = "noFeed";
  "noFeed" == currentSelectionId && emptyOption.setAttribute("selected", "");
...
GWDDynamicDataSelectorImpl.prototype.attributeChangedCallback = function() {
};
